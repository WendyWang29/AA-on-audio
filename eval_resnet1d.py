...
def init_eval(config, type_of_spec, epsilon, attack_model,
    ...
    if attack != 'Ens2D' and attack != 'Ens1D' and trip == 0:
        ...
        ResNet1D_eval(model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset, feature, q1, q2, q3, eps1, eps2, eps3)
    elif attack == 'Ens1D' and trip == 1:
        eps1_str = str(eps1).replace('.', 'dot')
        eps2_str = str(eps2).replace('.', 'dot')
        eps3_str = str(eps3).replace('.', 'dot')
        save_path = os.path.join(script_dir,
                                 'eval',
                                 f'probs_ResNet1D_{model_version}_{attack}_{attack_model}_{q1}_{q2}_{q3}_{eps1_str}_{eps2_str}_{eps3_str}_{type_of_spec}_{feature}.csv')

...
def init_eval(config, type_of_spec, epsilon, attack_model, model_version, attack, dataset, feature, q_res, q_sen):
    ...
    if attack != 'Ensemble' and attack != 'Ensemble1D':
        ...
        save_path = os.path.join(script_dir,
                                 'eval',
                                 f'probs_SENet_{model_version}_Ensemble1D_{dataset}_{q_res}_{q_sen}_{epsilon_str}_{type_of_spec}_{feature}.csv')
        SENet_eval(senet_model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset, feature,
                      q_res, q_sen)

...
def SENet_eval(resnet_model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset, feature, q_res, q_sen):
    epsilon_dot_notation = str(epsilon).replace('.', 'dot')
    ...
    with torch.no_grad():
        ...
        for feat_batch, utt_id in tqdm(feat_loader, total=len(feat_loader)):
            ...
            # score_list = []
            feat_batch = feat_batch.to(torch.float32).to(device)
            score = resnet_model(feat_batch.unsqueeze(dim=1))
            probabilities = torch.exp(score)
            probabilities = probabilities.detach().cpu().numpy()
...
def init_eval(config, type_of_spec, epsilon, attack_model, attack, dataset, feature, q_res, q_sen):
    ...
    if attack == None:
        ...
        if feature == 'audio' and dataset != '3s':
            ...
            save_path = f'eval/prob_SENet_{attack}_{attack_model}_{epsilon_str}.csv'
            SENet_eval(SENet_model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset,
                        feature, q_res, q_sen)
        elif feature == 'audio' and dataset == '3s':
            epsilon_str = str(epsilon).replace('.', 'dot')
            save_path = f'eval/prob_SENet_{attack}_{attack_model}_3s_{epsilon_str}_AUDIO.csv'
            SENet_eval(SENet_model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset,
                        feature, q_res, q_sen)
        elif feature == 'spec' and dataset == '3s':
            epsilon_str = str(epsilon).replace('.', 'dot')
            save_path = f'eval/prob_SENet_{attack}_{attack_model}_3s_{epsilon_str}_SPEC.csv'
            SENet_eval(SENet_model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset,
                        feature, q_res, q_sen)

    elif attack == 'Ensemble':
        epsilon_str = str(epsilon).replace('.', 'dot')
        save_path = f'eval/prob_SENet_Ensemble_{q_res}_{q_sen}_{epsilon_str}.csv'
        SENet_eval(SENet_model, save_path, device, config, type_of_spec, epsilon, attack_model, attack, dataset,
                    feature, q_res, q_sen)
